...
const SubCategory = () => {
    ...
    const fetchProducts = (page, platform, minPrice, maxPrice, type, region, title, sort, categoryId) => {
        axios.get(`${Base_url}/products/category/${id}?page=${page}&platform=${platform}&minPrice=${minPrice}&maxPrice=${maxPrice}&type=${type}&region=${region}&title=${title}&sort=${sort}&categoryId=${categoryId}`)
            .then((res) => {
