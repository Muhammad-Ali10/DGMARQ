...
const DgmarqFeatures = ({ title, data }) => {
    ...
    return (
        <>
            ...
            <div className="w-full  mx-auto flex flex-col justify-center gap-4 p-4 sm:p-5 md:px-8 md:py-6 lg:px-11 lg:py-6 xl:px-22 xl:py-13 bg-blue-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 w-full place-items-center gap-5 sm:gap-[20px] lg:gap-5 xl:gap-[25px]">
                    {data?.map((item, index) => (
                        <Card
                            key={index}
                            className="px-4 py-4 lg:px-4 lg:py-3 xl:px-4 xl:py-4 rounded-3xl flex flex-row items-start bg-blue border-0 w-full gap-3 sm:gap-2 text-white  h-full "
                        >
                            <img
                                src={item?.imageUrl}
                                alt={item?.title}
